...
public class QuestionController : Controller
{
    ...
    private readonly IGameTimer _timerService;

    public QuestionController(IMapper mapper,
        IQuestionService questionService,
        IConfiguration configuration, IGameTimer timerService)
    {
        _mapper = mapper;
    ...
    public IActionResult Create()
    {
        try
        {
            ...
            {
                new() { Id = Guid.NewGuid(), QuestionId = model.Id, IsCorrect = true },
                new() { Id = Guid.NewGuid(), QuestionId = model.Id, IsCorrect = false },
                new() { Id = Guid.NewGuid(), QuestionId = model.Id, IsCorrect = false },
                new() { Id = Guid.NewGuid(), QuestionId = model.Id, IsCorrect = false }
            };
    ...
    public async Task<IActionResult> CheckQuestionForCreateOrEdit(string text, Guid id)
    {
        ...
        async Task<bool> IsQuestionValidAsync(Guid questionId, string questionText)
        {
            ...
            if (!questionId.Equals(Guid.Empty))
            {
                ...
                if (isQuestionTheSame) return true;
            }
    ...
    [HttpPost]
    public async Task<IActionResult> SetGameTimer([FromQuery]int duration)
    {
        try
        {
            var timer = await _timerService.GetGameTimer();
            if (timer != null)
            {
                if (duration > 0)
                {
                    timer.Duration = duration;
                    await _timerService.UpdateTimer(timer);
                    return Json("OK");
                }
            }
            return Json("Error");
